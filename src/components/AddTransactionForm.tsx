...
export const AddTransactionForm = ({ loan, onSuccess, onCancel, defaultType = 'repaid' }: AddTransactionFormProps) => {
  ...
  const getDefaultTransactionType = (): 'borrowed_more' | 'repaid' | 'lent_more' => {
    if (defaultType === 'more') {
  ...
  const onSubmit = async (data: TransactionFormData) => {
    ...
    if (error) {
      ...
      const actionText = data.transaction_type === 'repaid' 
        ? 'Payment recorded' 
        : data.transaction_type === 'lent_more' 
          ? 'Additional loan recorded'
          : 'Additional borrowing recorded';
  ...
  // Get labels based on loan type
  const getTypeLabel = (type: 'borrowed_more' | 'repaid' | 'lent_more') => {
    if (type === 'repaid') {
  ...
  const getTypeDescription = (type: 'borrowed_more' | 'repaid' | 'lent_more') => {
    if (type === 'repaid') {
